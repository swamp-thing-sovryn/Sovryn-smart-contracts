...
describe("LiquidityMining", () => {
	...
	describe("Migration", () => {
		...
		describe("Grace period start", () => {
			it("should be deployed with state being before grace period", async () => {
				const migrationGracePeriodState = await liquidityMining.migrationGracePeriodState();
				expect(migrationGracePeriodState.toNumber()).to.equal(MigrationGracePeriodStates.None);
			});

			it("should only allow grace period to be started by the admin", async () => {
				await expectRevert(liquidityMining.startMigrationGracePeriod({ from: account1 }), "unauthorized");
			});

			it("should properly start grace period", async () => {
				await liquidityMining.startMigrationGracePeriod();

				const migrationGracePeriodState = await liquidityMining.migrationGracePeriodState();
				expect(migrationGracePeriodState.toNumber()).to.equal(MigrationGracePeriodStates.Started);

				const endBlock = await liquidityMining.endBlock();
				expect(endBlock).not.equal(0);
			});

			it("should fail when trying to start the grace period if already started", async () => {
				await liquidityMining.startMigrationGracePeriod();
				// check that the grace period is started
				const migrationGracePeriodState = await liquidityMining.migrationGracePeriodState();
				expect(migrationGracePeriodState.toNumber()).to.equal(MigrationGracePeriodStates.Started);

				await expectRevert(liquidityMining.startMigrationGracePeriod(), "Forbidden: migration already started");
			});

			it("should not let users to deposit", async () => {
				await liquidityMining.startMigrationGracePeriod();
				await expectRevert(
					liquidityMining.deposit(token1.address, new BN(10), ZERO_ADDRESS, { from: account1 }),
					"Forbidden: migration already started"
				);
			});

			it("should not let lending pools to deposit", async () => {
				await liquidityMining.startMigrationGracePeriod();
				await expectRevert(
					liquidityMining.onTokensDeposited(token1.address, new BN(10), { from: account1 }),
					"Forbidden: migration already started"
				);
			});

			it("should let users to update the pools information", async () => {
				let allocationPoint = new BN(1);
				await liquidityMining.add(token1.address, allocationPoint, false);
				await liquidityMining.startMigrationGracePeriod();
		...
		describe("Grace period finish", () => {
			it("should only allow admin to finish the grace period", async () => {
				await expectRevert(liquidityMining.finishMigrationGracePeriod({ from: account1 }), "unauthorized");
			});
			...
			it("should properly finish grace period", async () => {
				await liquidityMining.startMigrationGracePeriod();
				// check that the grace period is started
				let migrationGracePeriodState = await liquidityMining.migrationGracePeriodState();
				expect(migrationGracePeriodState.toNumber()).to.equal(MigrationGracePeriodStates.Started);

				await liquidityMining.finishMigrationGracePeriod();
				// check that the grace period is finished
				migrationGracePeriodState = await liquidityMining.migrationGracePeriodState();
				expect(migrationGracePeriodState.toNumber()).to.equal(MigrationGracePeriodStates.Finished);
			});

			it("should not let users to withdraw", async () => {
				await liquidityMining.startMigrationGracePeriod();
				await liquidityMining.finishMigrationGracePeriod();
				await expectRevert(liquidityMining.withdraw(token1.address, new BN(10), account1), "Forbidden: contract deprecated");
				await expectRevert(liquidityMining.emergencyWithdraw(token1.address), "Forbidden: contract deprecated");
			});

			it("should not let users to claim rewards", async () => {
				await liquidityMining.startMigrationGracePeriod();
				await liquidityMining.finishMigrationGracePeriod();
				await expectRevert(liquidityMining.claimRewardFromAllPools(account1), "Forbidden: contract deprecated");

				await expectRevert(liquidityMining.claimReward(token1.address, account1), "Forbidden: contract deprecated");
			});

			it("should not let users to update the pools information", async () => {
				let allocationPoint = new BN(1);
				await liquidityMining.add(token1.address, allocationPoint, false);

				await liquidityMining.startMigrationGracePeriod();
				await liquidityMining.finishMigrationGracePeriod();
				await expectRevert(liquidityMining.updatePool(token1.address), "Forbidden: contract deprecated");
				await expectRevert(liquidityMining.updateAllPools(), "Forbidden: contract deprecated");
			});
		});
	});
...
describe("Contract upgrade", async () => {
	...
	describe("emergencyWithdraw", () => {
		...
		it("should be able to withdraw", async () => {
			...
			const afterBlockNumber = await web3.eth.getBlockNumber();
