...
describe("LiquidityMining", () => {
	...
	describe("stopMining", () => {
		...
		it("fails if not an owner or an admin", async () => {
			await expectRevert(liquidityMining.stopMining(SOVToken.address, { from: account1 }), "unauthorized");
	...
	describe("transferRewardTokens", () => {
		...
		it("fails if the zero address is passed as token", async () => {
			await expectRevert(liquidityMining.transferRewardTokens(ZERO_ADDRESS, account1, 0), "Reward address invalid");
		});
	});
	...
	describe("add", () => {
		it("should be able to add pool token", async () => {
			...
			let tx = await liquidityMining.add(token1.address, [SOVToken.address], [allocationPoint], false);
		...
		it("should be able to add 2 pool tokens and update pools", async () => {
			...
			let tx1 = await liquidityMining.add(token1.address, [SOVToken.address], [allocationPoint1], false);
			...
			expect(rewardToken.totalAllocationPoint).bignumber.equal(allocationPoint1.add(allocationPoint2));
		...
		it("fails if the reward token list is empty", async () => {
			await expectRevert(liquidityMining.add(token1.address, [], [new BN(0)], false), "Invalid reward tokens length");
		});

		it("fails if the reward tokens length is different to allocation points", async () => {
			await expectRevert(liquidityMining.add(token1.address, [token2.address, token3.address], [new BN(1)], false), "Invalid allocation points length");
		});

		it("fails if the reward token is repeated", async () => {
			await expectRevert(liquidityMining.add(token1.address, [SOVToken.address, SOVToken.address], [new BN(1), new BN(1)], false), "Already associated");
		});

		it("fails if the reward token is not valid", async () => {
			await expectRevert(liquidityMining.add(token1.address, [token2.address], [new BN(1)], false), "Not initialized");
		});
		...
		it("fails if the 0 address is passed as token address", async () => {
			await expectRevert(liquidityMining.add(ZERO_ADDRESS, [SOVToken.address], [new BN(0)], false), "Invalid token address");
		});

		it("fails if token already added", async () => {
			await liquidityMining.add(token1.address, [SOVToken.address], [new BN(1)], false);
			await expectRevert(liquidityMining.add(token1.address, [SOVToken.address], [new BN(0)], false), "Token already added");
		});

		it("only owner or admin should be able to add pool token", async () => {
			await expectRevert(liquidityMining.add(token2.address, [SOVToken.address], [new BN(0)], false, { from: account1 }), "unauthorized");

			await liquidityMining.addAdmin(account1);
			await liquidityMining.add(token2.address, [SOVToken.address], [new BN(1)], false, { from: account1 });
		});
	});
	...
	describe("updateTokens", () => {
		it("should be able to update 2 pool tokens", async () => {
			...
			for (let i = 0; i < poolTokens.length; i++) {
				await liquidityMining.add(poolTokens[i], [SOVToken.address], [oldAllocationPoints[i]], false);
			}

			let newAllocationPoints = [[new BN(101)], [new BN(102)], [new BN(3)]];
			let rewardTokens = new Array(3).fill([SOVToken.address]);
			let tx = await liquidityMining.updateTokens(poolTokens, rewardTokens, newAllocationPoints, true);
			...
			for (let i = 0; i < poolTokens.length - 1; i++) {
				...
				const poolRewardToken = await liquidityMining.getPoolReward(poolTokens[i], SOVToken.address);
				checkPoolRewardInfo(poolInfo, poolTokens[i], poolRewardToken, newAllocationPoints[i][0], blockNumber, new BN(0));
			...
			const poolRewardToken = await liquidityMining.getPoolReward(poolTokens[poolTokens.length - 1], SOVToken.address);
			expect(poolRewardToken.lastRewardBlock).bignumber.equal(blockNumber);
		});

		it("fails if token wasn't added", async () => {
			await expectRevert(liquidityMining.updateTokens([token1.address], [[SOVToken.address]], [[new BN(1)]], false), "Pool token not found");
		});

		it("fails if arrays have different length", async () => {
			await liquidityMining.add(token2.address, [SOVToken.address], [new BN(1)], false);
			await expectRevert(liquidityMining.updateTokens([token1.address, token2.address], [[SOVToken.address]], [[new BN(1)]], false), "Arrays mismatch");

			await expectRevert(liquidityMining.updateTokens([token1.address, token2.address], [[SOVToken.address]], [[new BN(1)], [new BN(1)]], false), "Arrays mismatch");
			
			await expectRevert(liquidityMining.updateTokens([token1.address, token2.address], [[SOVToken.address, token3.address], [SOVToken.address]], [[new BN(1)], [new BN(1)]], false), "Arrays mismatch");

			await expectRevert(liquidityMining.updateTokens([token1.address, token2.address], [[SOVToken.address, token3.address]], [[new BN(1)], [new BN(1)]], false), "Arrays mismatch");
		});

		it("only owner or admin should be able to update pool token", async () => {
			await liquidityMining.add(token2.address, [SOVToken.address], [new BN(1)], false);
			await expectRevert(liquidityMining.updateTokens([token2.address], [[SOVToken.address]], [[new BN(1)]], false, { from: account1 }), "unauthorized");

			await liquidityMining.addAdmin(account1);
			await liquidityMining.updateTokens([token2.address], [[SOVToken.address]], [[new BN(1)]], false, { from: account1 });
		});
	});
	...
	describe("deposit", () => {
		...
		it("should be able to deposit", async () => {
			...
			let blockNumber = new BN(tx.receipt.blockNumber);
			const poolRewardToken = await liquidityMining.getPoolReward(token1.address, SOVToken.address);
			checkPoolRewardInfo(poolInfo, token1.address, poolRewardToken, allocationPoint, blockNumber, new BN(0));
		...
		it("should be able to deposit 0 amount", async () => {
			let tx = await liquidityMining.deposit(token1.address, new BN(0), ZERO_ADDRESS, { from: account1 });
	...
	describe("claimRewards", () => {
		...
		it("shouldn't be able to claim reward (will not be claimed without SOV tokens)", async () => {
			...
			await expectRevert(liquidityMining.claimRewards(token1.address, ZERO_ADDRESS, { from: account1 }), "Claiming reward failed");
		});
		...
		it("should be able to claim reward (will be claimed with SOV tokens)", async () => {
			...
			let tx = await liquidityMining.claimRewards(token1.address, ZERO_ADDRESS, { from: account1 });
		...
		it("should not take into account blocks before start delay", async () => {
			...
			await liquidityMining.addRewardToken(token3.address, new BN(3), new BN(5), erc20RewardTransferLogic.address);
			...
			const reward1 = await liquidityMining.getUserAccumulatedReward(token2.address, token3.address, account1);
			expect(reward1).bignumber.equal(new BN(0));
			
			await mineBlock();

			let tx = await liquidityMining.claimRewards(token2.address, ZERO_ADDRESS, { from: account1 });
		...
		it("should not take into account blocks after stop mining", async () => {
			...
			await erc20RewardTransferLogic.initialize(token3.address);
			await liquidityMining.addRewardToken(token3.address, new BN(3), new BN(2), erc20RewardTransferLogic.address);
		...
		it("fails if token pool token not found", async () => {
			await expectRevert(liquidityMining.claimRewards(account1, ZERO_ADDRESS, { from: account1 }), "Pool token not found");
		});
	});
	...
	describe("claimRewardFromAllPools", () => {
		...
		beforeEach(async () => {
			await liquidityMining.add(token1.address, [SOVToken.address], [allocationPoint], false);
			await liquidityMining.add(token2.address, [SOVToken.address], [allocationPoint], false);
			await mineBlocks(1);
	...
	describe("withdraw", () => {
		...
		it("fails if withdraw for a user without using wrapper or pool", async () => {
			await expectRevert(liquidityMining.withdraw(token1.address, amount, account1, { from: account3 }), "only wrapper or pools may withdraw for a user");
		});
	...
	describe("getUserAccumulatedReward", () => {
		...
		beforeEach(async () => {
			await liquidityMining.add(token1.address, [SOVToken.address], [allocationPoint1], false);
			await liquidityMining.add(token2.address, [SOVToken.address], [allocationPoint2], false);
		...
		it("check calculation for no deposits", async () => {
			...
			const reward2 = await liquidityMining.getUserAccumulatedReward(token2.address, SOVToken.address, account2);
		...
		it("check calculation for single user, token 2", async () => {
			...
			await mineBlock();
			let reward = await liquidityMining.getUserAccumulatedReward(token2.address, SOVToken.address, account2);
			...
			await mineBlock();
			reward = await liquidityMining.getUserAccumulatedReward(token2.address, SOVToken.address, account2);
		...
		it("check calculation for two users and tokens", async () => {
			...
			await liquidityMining.deposit(token2.address, amount2, ZERO_ADDRESS, { from: account2 });
			// sanity checks
			expect(await liquidityMining.getUserAccumulatedReward(token1.address, SOVToken.address, account1)).bignumber.equal("1");
			expect(await liquidityMining.getUserAccumulatedReward(token2.address, SOVToken.address, account2)).bignumber.equal("0");
			await mineBlock();
		...
		it("check calculation for two users, same token (shares taken into account)", async () => {
			...
			// sanity checks
			expect(await liquidityMining.getUserAccumulatedReward(token.address, SOVToken.address, account1)).bignumber.equal("1");
			expect(await liquidityMining.getUserAccumulatedReward(token.address, SOVToken.address, account2)).bignumber.equal("0");
			await mineBlock();
			await mineBlock();

			const reward1 = await liquidityMining.getUserAccumulatedReward(token.address, SOVToken.address, account1);
			const reward2 = await liquidityMining.getUserAccumulatedReward(token.address, SOVToken.address, account2);
	...
	describe("getEstimatedReward", () => {
		...
		it("check calculation for 1 user 2 delayed block reward, period less than 2 blocks", async () => {
			...
			let estimatedReward = await liquidityMining.getEstimatedReward(token2.address, token3.address, amount3, duration);
		...
		it("check calculation for 1 user, period less than 1 block", async () => {
			...
			let estimatedReward = await liquidityMining.getEstimatedReward(token1.address, SOVToken.address, amount3, duration);
			let expectedReward = "0";
			expect(estimatedReward).bignumber.equal(expectedReward);
		});
	...
	describe("deposit/withdraw", () => {
		...
		it("add, add, deposit, deposit", async () => {
			...
			const expectedAccumulatedRewardPerBlock = rewardTokensPerBlock.mul(new BN(1e12)).div(new BN(2)).div(amount);
			
			const poolInfo1 = await liquidityMining.getPoolInfo(token1.address);
			const poolReward = await liquidityMining.getPoolReward(token1.address, SOVToken.address);
			...
			expect(poolReward.accumulatedRewardPerShare).equal(expectedAccumulatedRewardPerBlock.mul(new BN(2)).toString());

			const poolInfo2 = await liquidityMining.getPoolInfo(token2.address);
			const poolReward2 = await liquidityMining.getPoolReward(token2.address, SOVToken.address);
			...
			expect(poolReward2.accumulatedRewardPerShare).equal(expectedAccumulatedRewardPerBlock.toString());
		});
	...
	describe("LM configuration", () => {
		...
		it("dummy pool + 1 pool", async () => {
			...
			await liquidityMining.add(SOVBTCpool, [SOVToken.address], [ALLOCATION_POINT_SOV_BTC], false); //weight 40000 / 100000
			await liquidityMining.add(dummyPool, [SOVToken.address], [MAX_ALLOCATION_POINT.sub(ALLOCATION_POINT_SOV_BTC)], false); //weight (100000 - 40000) / 100000
		...
		it("dummy pool + 2 pools", async () => {
			...
			await liquidityMining.add(dummyPool, [SOVToken.address], [DUMMY_ALLOCATION_POINT], false); //weight (100000 - 40000) / 100000
			...
			await liquidityMining.update(SOVBTCpool, [SOVToken.address], [ALLOCATION_POINT_SOV_BTC_2], false); //weight 30000 / 100000

			await liquidityMining.add(ETHBTCpoll, [SOVToken.address], [ALLOCATION_POINT_ETH_BTC], false); //weight 37500 / 100000
			const DUMMY_ALLOCATION_POINT_2 = MAX_ALLOCATION_POINT.sub(ALLOCATION_POINT_SOV_BTC_2).sub(ALLOCATION_POINT_ETH_BTC);
			await liquidityMining.update(dummyPool, [SOVToken.address], [DUMMY_ALLOCATION_POINT_2], false); //weight (100000 - 30000 - 37500) / 100000
			await liquidityMining.updateAllPools();
	...
	describe("multiple rewards tokens per pool", () => {
		...
		it("check rewards for two reward tokens and one pool", async () => {
			...
			const claimRewardTx1 = await liquidityMining.claimReward(token1.address, rewardToken1.address, ZERO_ADDRESS, { from: account1 });
			...
			const claimRewardTx2 = await liquidityMining.claimReward(token1.address, rewardToken2.address, ZERO_ADDRESS, { from: account1 });
	...
	describe("external getters", () => {
		...
		it("totalAllocationPoint", async () => {
			...
			await liquidityMining.add(token2.address, [SOVToken.address], [allocationPoint], false);
			rewardToken = await liquidityMining.getRewardToken(SOVToken.address);
			expect(rewardToken.totalAllocationPoint).bignumber.equal(allocationPoint.mul(new BN(2)));
		});
		...
		it("getUserAccumulatedReward", async () => {
			...
			await mineBlock();
			const reward1 = await liquidityMining.getUserAccumulatedReward(token1.address, SOVToken.address, account1);
			const reward2 = await liquidityMining.getUserAccumulatedReward(token1.address, SOVToken.address, account2);
		...
		it("getPoolInfoList", async () => {
			...
			const poolRewardInfo = await liquidityMining.getPoolReward(token1.address, info.rewardTokens[0]);
			...
			expect(poolRewardInfo.lastRewardBlock).equal((await web3.eth.getBlockNumber()).toString());
		});
